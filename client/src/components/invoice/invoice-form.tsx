...
import { calculateInvoiceTotal, formatCurrency } from "@/lib/utils-invoice";
...
interface InvoiceFormProps {
  ...
  onSubmit: (data: { invoice: Omit<InsertInvoice, 'invoiceNumber'>; items: InvoiceItem[] }) => void;
  onCancel: () => void;
  isLoading?: boolean;
  submitText?: string;
}
...
export default function InvoiceForm({ 
  ...
  const [formData, setFormData] = useState<Omit<InsertInvoice, 'invoiceNumber'>>({
    ...
    vatAmount: initialData?.vatAmount || "0.00",
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    onSubmit({ invoice: formData, items });
  };
