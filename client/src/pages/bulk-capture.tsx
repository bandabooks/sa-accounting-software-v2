...
const EnhancedBulkCapture = () => {
  ...
  const saveExpensesMutation = useMutation({
    mutationFn: async () => {
      ...
      const numberResponse = await fetch('/api/bulk-capture/next-number/expense', {
        headers: {
      ...
      const entryNumber = numberData.nextNumber;
      ...
      for (let i = 0; i < validEntries.length; i++) {
        ...
        if (i > 0) {
          const nextNumResponse = await fetch('/api/bulk-capture/next-number/expense', {
            headers: {
              'Content-Type': 'application/json',
            },
            credentials: 'include'
          ...
          currentEntryNumber = nextNumData.nextNumber;
        }
  ...
  const saveIncomesMutation = useMutation({
    mutationFn: async () => {
      ...
      const numberResponse = await fetch('/api/bulk-capture/next-number/income', {
        headers: {
          'Content-Type': 'application/json',
        },
        credentials: 'include'
      ...
      for (let i = 0; i < validEntries.length; i++) {
        ...
        let currentEntryNumber = entryNumber;
        if (i > 0) {
          const nextNumResponse = await fetch('/api/bulk-capture/next-number/income', {
            headers: {
              'Content-Type': 'application/json',
            },
            credentials: 'include'
    ...
    <div className="space-y-6 p-6">
      ...
      <div className={`p-6 rounded-lg ${
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-3">
            {/* Income Summary Cards - only show when income tab is active */}
            {activeTab === 'income' && (
              <div className="flex items-center space-x-3">
                {/* Total Income Card */}
                <div className="bg-green-600 px-4 py-2 rounded-lg">
                  <div className="text-xs font-medium text-green-100">TOTAL INCOME</div>
                ...
                <div className="bg-blue-600 px-4 py-2 rounded-lg">
                  <div className="text-xs font-medium text-blue-100">VAT AMOUNT</div>
                  <div className="text-lg font-bold text-white">
                ...
                <div className="bg-emerald-600 px-4 py-2 rounded-lg">
                  <div className="text-xs font-medium text-emerald-100">NET INCOME</div>
      ...
      <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as 'income' | 'expense' | 'bank-import')}>
        ...
        <TabsContent value="income" className="space-y-6">
          ...
          {/* VAT Breakdown - Positioned as single compact card for space efficiency */}
          <div className="grid grid-cols-1 gap-3">
            <Card className="bg-gradient-to-br from-purple-50 to-indigo-50 border-purple-200 max-w-md">
              ...
              <CardContent className="pt-0">
                <div className="grid grid-cols-5 gap-2 text-center">
                  <div>
                    <div className="text-xs text-blue-600 font-medium">VAT Inc</div>
                  ...
                  <div>
                    <div className="text-xs text-yellow-600 font-medium">Zero</div>
                    <div className="text-xs text-gray-800 font-semibold">R {incomeCalculations.vatBreakdown.zeroRated.toFixed(0)}</div>
                  </div>
                  <div>
                    <div className="text-xs text-orange-600 font-medium">Exempt</div>
                    <div className="text-xs text-gray-800 font-semibold">R {incomeCalculations.vatBreakdown.exempt.toFixed(0)}</div>
                  </div>
                  <div>
                    <div className="text-xs text-red-600 font-medium">No VAT</div>
                    <div className="text-xs text-gray-800 font-semibold">R {incomeCalculations.vatBreakdown.noVAT.toFixed(0)}</div>
                  </div>
                </div>
              </CardContent>
            </Card>
