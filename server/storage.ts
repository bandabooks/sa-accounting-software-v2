...
import { db } from "./db";
import { eq, desc, sum, count, sql, and, gte, lte, lt, or, isNull } from "drizzle-orm";
...
export class DatabaseStorage implements IStorage {
  ...
  async updateBankAccount(id: number, updateData: Partial<InsertBankAccount>): Promise<BankAccount | undefined> {
    const [account] = await db
      .update(bankAccounts)
      .set(updateData)
  ...
  async getBankTransactions(companyId: number, bankAccountId?: number): Promise<BankTransaction[]> {
  ...
  async createBankTransaction(insertTransaction: InsertBankTransaction): Promise<BankTransaction> {
    return await db.transaction(async (tx) => {
      ...
      // Create corresponding journal entry for double-entry
      if (insertTransaction.bankAccountId) {
        const bankAccount = await this.getBankAccount(insertTransaction.bankAccountId);
        if (bankAccount?.chartAccountId) {
          const entryNumber = await this.generateJournalEntryNumber(insertTransaction.companyId);
          
          const lines: InsertJournalEntryLine[] = [];
          
          if (insertTransaction.transactionType === 'deposit' || insertTransaction.transactionType === 'receipt') {
            // Debit Bank Account, Credit will be determined by source
            lines.push({
              journalEntryId: 0, // Will be set after journal entry creation
              accountId: bankAccount.chartAccountId,
              description: insertTransaction.description,
              debitAmount: insertTransaction.amount,
              creditAmount: "0.00",
              reference: insertTransaction.reference || '',
            });
          } else if (insertTransaction.transactionType === 'withdrawal' || insertTransaction.transactionType === 'payment') {
            // Credit Bank Account, Debit will be determined by destination
            lines.push({
              journalEntryId: 0,
              accountId: bankAccount.chartAccountId,
              description: insertTransaction.description,
              debitAmount: "0.00",
              creditAmount: insertTransaction.amount,
              reference: insertTransaction.reference || '',
            });
          }
          ...
          if (lines.length > 0) {
            const journalEntry: InsertJournalEntry = {
              ...
              description: `Bank ${insertTransaction.transactionType}: ${insertTransaction.description}`,
              reference: insertTransaction.reference,
              totalDebit: insertTransaction.transactionType === 'deposit' || insertTransaction.transactionType === 'receipt' ? insertTransaction.amount : "0.00",
              totalCredit: insertTransaction.transactionType === 'withdrawal' || insertTransaction.transactionType === 'payment' ? insertTransaction.amount : "0.00",
              sourceModule: 'banking',
              sourceId: transaction.id,
              createdBy: insertTransaction.createdBy,
            };

            const createdEntry = await this.createJournalEntry(journalEntry, lines);
            
            // Update transaction with journal entry reference
            await tx
              .update(bankTransactions)
              .set({ journalEntryId: createdEntry.id })
  ...
  async getInventoryCategories(companyId: number): Promise<InventoryCategory[]> {
    return await db.select().from(inventoryCategories)
      .where(and(eq(inventoryCategories.companyId, companyId), eq(inventoryCategories.isActive, true)))
  ...
  async getInventoryItems(companyId: number, categoryId?: number): Promise<InventoryItem[]> {
    let query = db.select().from(inventoryItems)
      .where(and(eq(inventoryItems.companyId, companyId), eq(inventoryItems.isActive, true)));
    
    if (categoryId) {
      query = query.where(eq(inventoryItems.categoryId, categoryId));
    }
    
    return await query.orderBy(inventoryItems.itemName);
  }
  ...
  async createInventoryMovement(insertMovement: InsertInventoryMovement): Promise<InventoryMovement> {
    return await db.transaction(async (tx) => {
      ...
      if (item) {
        let newQuantity = parseFloat(item.quantityOnHand);
        
        if (insertMovement.movementType === 'purchase' || insertMovement.movementType === 'adjustment') {
          newQuantity += parseFloat(insertMovement.quantity);
        } else if (insertMovement.movementType === 'sale') {
          newQuantity -= parseFloat(insertMovement.quantity);
        }
        ...
        await tx
          ...
          .set({ 
            ...
            averageCost: this.calculateAverageCost(parseFloat(item.averageCost), parseFloat(insertMovement.unitCost), parseFloat(item.quantityOnHand), parseFloat(insertMovement.quantity), insertMovement.movementType).toString(),
            updatedAt: new Date(),
          })
        ...
        if (item.trackInventory) {
          ...
          if (category[0] && category[0].assetAccountId) {
            const entryNumber = await this.generateJournalEntryNumber(insertMovement.companyId);
            const lines: InsertJournalEntryLine[] = [];

            if (insertMovement.movementType === 'purchase') {
              // Debit Inventory Asset, Credit will be handled by purchase order or expense
              lines.push({
                journalEntryId: 0,
                accountId: category[0].assetAccountId,
                description: `Inventory purchase: ${item.itemName}`,
                debitAmount: insertMovement.totalCost,
                creditAmount: "0.00",
                reference: insertMovement.reference || '',
              });
            } else if (insertMovement.movementType === 'sale') {
              // Credit Inventory Asset, Debit COGS
              lines.push({
                journalEntryId: 0,
                accountId: category[0].assetAccountId,
                description: `Inventory sale: ${item.itemName}`,
                debitAmount: "0.00",
                creditAmount: insertMovement.totalCost,
                reference: insertMovement.reference || '',
              });

              if (category[0].cogsAccountId) {
                lines.push({
                  journalEntryId: 0,
                  accountId: category[0].cogsAccountId,
            ...
            if (lines.length > 0) {
              const totalDebit = lines.reduce((sum, line) => sum + parseFloat(line.debitAmount), 0).toString();
              const totalCredit = lines.reduce((sum, line) => sum + parseFloat(line.creditAmount), 0).toString();

              const journalEntry: InsertJournalEntry = {
                companyId: insertMovement.companyId,
                entryNumber,
                transactionDate: new Date(insertMovement.movementDate),
                description: `Inventory ${insertMovement.movementType}: ${item.itemName}`,
                reference: insertMovement.reference,
                totalDebit,
                totalCredit,
  ...
  private calculateAverageCost(currentAvgCost: number, newCost: number, currentQty: number, newQty: number, movementType: string): number {
    if (movementType === 'purchase' || movementType === 'adjustment') {
      const totalValue = (currentAvgCost * currentQty) + (newCost * newQty);
  ...
  async getInventoryMovements(companyId: number, itemId?: number): Promise<InventoryMovement[]> {
    ...
    return await query.orderBy(desc(inventoryMovements.movementDate));
  }

  // Service Module
  async getServiceCategories(companyId: number): Promise<ServiceCategory[]> {
    return await db.select().from(serviceCategories)
      .where(and(eq(serviceCategories.companyId, companyId), eq(serviceCategories.isActive, true)))
  ...
  async getServices(companyId: number, categoryId?: number): Promise<Service[]> {
    ...
    return await query.orderBy(services.serviceName);
  }
  ...
  async generateTrialBalance(companyId: number, periodStart: string, periodEnd: string): Promise<AccountBalanceReport[]> {
    const accounts = await db.select().from(chartOfAccounts)
      .where(and(eq(chartOfAccounts.companyId, companyId), eq(chartOfAccounts.isActive, true)))
      .orderBy(chartOfAccounts.accountCode);

    const trialBalanceData: AccountBalanceReport[] = [];

    for (const account of accounts) {
      // Calculate opening balance (transactions before period start)
      const openingBalanceQuery = await db
        .select({
          debits: sql<string>`COALESCE(SUM(${journalEntryLines.debitAmount}), 0)`,
          credits: sql<string>`COALESCE(SUM(${journalEntryLines.creditAmount}), 0)`,
        })
        .from(journalEntryLines)
        .innerJoin(journalEntries, eq(journalEntryLines.journalEntryId, journalEntries.id))
        .where(and(
          ...
          lt(journalEntries.transactionDate, new Date(periodStart)),
          eq(journalEntries.isPosted, true)
        ));

      // Calculate period movements
      const periodQuery = await db
        .select({
          debits: sql<string>`COALESCE(SUM(${journalEntryLines.debitAmount}), 0)`,
          credits: sql<string>`COALESCE(SUM(${journalEntryLines.creditAmount}), 0)`,
        })
        ...
        .where(and(
          eq(journalEntryLines.accountId, account.id),
          eq(journalEntries.companyId, companyId),
          gte(journalEntries.transactionDate, new Date(periodStart)),
          lte(journalEntries.transactionDate, new Date(periodEnd)),
          eq(journalEntries.isPosted, true)
        ));

      const openingDebits = parseFloat(openingBalanceQuery[0]?.debits || "0");
      const openingCredits = parseFloat(openingBalanceQuery[0]?.credits || "0");
      const periodDebits = parseFloat(periodQuery[0]?.debits || "0");
      const periodCredits = parseFloat(periodQuery[0]?.credits || "0");
      ...
      trialBalanceData.push({
        ...
        debitTotal: periodDebits.toFixed(2),
        creditTotal: periodCredits.toFixed(2),
