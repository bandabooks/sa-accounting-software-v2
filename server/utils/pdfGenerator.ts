// Try different import approach for jsPDF
let jsPDF: any;
try {
...
export function generateVatSummaryPDF(data: VatSummaryData, companyName: string = 'MY Redeployment'): Buffer {
  console.log('Creating PDF, jsPDF type:', typeof jsPDF);
  ...
  try {
    ...
    // Basic summary info without complex tables initially
    pdf.setFontSize(14);
    pdf.text('VAT Summary:', 20, 70);
    
    pdf.setFontSize(12);
    pdf.text(`Output VAT: R ${data.summary.outputVat || '0.00'}`, 20, 85);
    pdf.text(`Input VAT: R ${data.summary.inputVat || '0.00'}`, 20, 100);
    pdf.text(`Net VAT Payable: R ${data.summary.netVatPayable || '0.00'}`, 20, 115);
    
    pdf.text('Transaction Count:', 20, 140);
    pdf.text(`Invoices: ${data.transactions?.invoiceCount || 0}`, 20, 155);
    pdf.text(`Expenses: ${data.transactions?.expenseCount || 0}`, 20, 170);

    // Footer
    pdf.setFontSize(8);
    pdf.setTextColor(150, 150, 150);
    pdf.text('Generated by Taxnify Business Management Platform', 20, 280);
    pdf.text('Page 1 of 1', 190, 280, { align: 'right' });

    const pdfOutput = pdf.output('arraybuffer');
    console.log('PDF generated successfully, size:', pdfOutput.byteLength);
